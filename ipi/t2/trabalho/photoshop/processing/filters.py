...
def apply_filter(img: np.ndarray, kernel: np.ndarray,
    ...
    """Generic filter: correlation (default) or convolution (kernel rotated 180°)."""
...
def box_filter(img: np.ndarray, ksize: int = 3,
    ...
    """Box (mean) filter. Separable path: two 1D passes for performance."""
    if separable:
...
def gaussian_filter(img: np.ndarray, sigma: float = 1.0,
                    padding: str = 'reflect', separable: bool = True,
                    convolve: bool = False) -> np.ndarray:
    """Gaussian filter. Kernel size = ceil(6*sigma) rounded to odd."""
...
def laplacian_sharpen(img: np.ndarray, kernel_type: str = '8-conn',
    ...
    """Laplacian sharpening: g = f + c*Lap(f).
    kernel_type: '4-conn'|'8-conn'|'4-conn+'|'8-conn+'
    Negative-center kernels use c<0; positive-center use c>0.
    """
    kernels = {
...
def sobel_sharpen(img: np.ndarray, c: float = 0.5,
    ...
    """Sobel gradient sharpening: g = f + c*M(x,y), M = sqrt(Gx²+Gy²)."""
...
def unsharp_mask(img: np.ndarray, sigma: float = 2.0,
                 c: float = 1.0, padding: str = 'reflect') -> np.ndarray:
    """Unsharp masking: o = f + c*(f - smooth(f)).
    1. Smooth f to get f_bar
    2. mask = f - f_bar  (detail layer)
    3. output = f + c*mask
    """
